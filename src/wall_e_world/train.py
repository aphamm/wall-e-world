...
def main(
    ...
    optimizer = optim.AdamW(
        model.parameters(), lr=lr, weight_decay=0.002, betas=(0.9, 0.99)
    )
    ...
    while train_steps < max_train_steps:
        ...
        if train_steps == 0 or (train_steps + 1) % log_every == 0:
            ...
            # Sync after rank 0 writes files to prevent desync
            if distributed:
                torch.cuda.synchronize()  # Ensure all GPU operations complete
                dist.barrier()  # Wait for all other ranks

            running_loss.zero_()
            num_batches = 0

        # Only rank 0 does validation, but all ranks must wait here to prevent DDP gradient sync deadlock
        if train_steps == 0 or train_steps % validate_every == 0:
            if rank == 0:
                model.eval()
                with torch.no_grad():
                    try:
                ...
                summary_writer.add_video(
                    f"val/generation_cfg{cfg}",
                    video_np,
                    train_steps,
                    dataformats="NTHWC",
                )
                summary_writer.add_video(
                    "val/gt", gt_np, train_steps, dataformats="NTHWC"
                )
