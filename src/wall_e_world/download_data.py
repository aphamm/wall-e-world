...
def process_single_episode(
    args: Tuple[int, Dict[str, Any], Callable, Path, int]
) -> Tuple[int, bool, str]:
    """Process a single episode - designed for parallel execution.

    Args:
        args: Tuple of (index, episode_data, step_map_fn, split_output_dir, fps)

    Returns:
        Tuple of (index, success, error_message)
    """
    i, episode_data, step_map_fn, split_output_dir, fps = args
    try:
        # Map episode to target format
        episode = episode_map_fn(episode_data, map_step=step_map_fn)
...
def convert_dataset(
    ...
    for split in ["train", "test"]:
        ...
        print(f"Processing {split} split with {num_workers} workers...")

        if num_workers <= 1:
            # Sequential processing (original behavior)
            for i, episode in tqdm(enumerate(dataset), desc=f"{dataset_name}-{split}"):
                _, success, error = process_single_episode(
                    (i, episode, dataset_config["step_map_fn"], split_output_dir, fps)
                )
                if not success:
                    print(f"Failed to process episode {i} in {dataset_name}-{split}: {error}")
        else:
            # Parallel processing: collect batches, then process in parallel
            step_map_fn = dataset_config["step_map_fn"]
            batch_size = num_workers * 2  # Process 2x workers at a time
            completed = 0
            failed = 0

            batch = []
            pbar = tqdm(desc=f"{dataset_name}-{split}")

            for i, episode in enumerate(dataset):
                batch.append((i, episode, step_map_fn, split_output_dir, fps))

                # Process batch when full
                if len(batch) >= batch_size:
                    with ThreadPoolExecutor(max_workers=num_workers) as executor:
                        results = list(executor.map(process_single_episode, batch))
                    for idx, success, error in results:
                        if success:
                            completed += 1
                        else:
                            failed += 1
                            print(f"Failed episode {idx}: {error}")
                        pbar.update(1)
                    batch = []
...
def main(
    ...
    """Download and convert robotics datasets to MP4+NPZ format.

    Args:
        dataset_name: Name of dataset to download, or 'all' for all datasets
        output_dir: Directory to save converted files
        dataset_home: Base path for dataset sources
